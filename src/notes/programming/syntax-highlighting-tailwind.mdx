...
At this point, I wanted to have a custom component that provided syntax highlighting for bits of code. Note that you can't pass custom components directly to `MDXRenderer`, so you'll need to wrap it in an `MDXProvider` component which you can pass your custom components to. The revised note template should end up looking a bit like this:
...
Here's a breakdown of what's doing on up there:
