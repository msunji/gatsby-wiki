...
For my personal wiki, I wanted to have all my notes displayed in the `Sidebar` component, but organised by category (see image above). Each category can then be expanded to view all the notes that fall under that category. Pretty straightforward stuff.

## Shaping your data with GraphQL

Quick note: Since I needed to pull data into a component and not a page, I used a `useStaticQuery` hook.
