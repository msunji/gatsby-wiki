---
title: Automating Note Creation with Plop
...
Enter [Plop](https://github.com/plopjs/plop) - a *micro-generator framework* that lets you generate code and other flat text files easily.
...
Here's the plopfile - `plopfile.js` in its entirety:
...
In my case, I want to make a generator that makes a Markdown file for me. I'd like to be able to set the title and specify which category to make the file in - see the `category` and `title` prompts shown above.

Now I only really need to perform one action: *add* a new note in a particular category. The `path` property is basically where the new file gets rendered, while the `templateFile` property's the path to the handlebars template file.
...
To generate a new note file, run `npm run plop`, answer the prompts, and enjoy the time you've saved by automating this task ✨
