...
Great. With that out of the way, it's time to get my notes prettied up. I opted to go with Tailwind's official Typography plugin to get my notes styled out of the box. It's pretty simple to use and you only need to add the `prose` class to get it working. Code blocks and inline blocks get styled accordingly as well, but it really isn't anything too fancy.
